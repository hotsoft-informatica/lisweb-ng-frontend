...
export class UnidadeMedidaComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.currentRecord = new UnidadeMedida({});
    this.record ||= new UnidadeMedida({});
  }

  ngOnInit(): void {
    this.unidadeMedidaService.count().subscribe((totalCount) => {
  ...
  addGridData(): void {
    ...
    this.onCreate = false;
    this.onEdit = false;
    this.unidadeMedidaService.create(this.currentRecord).subscribe((record) => {
      this.records.unshift(record);
      this.datasource.data = [...this.records];
      this.unidadeMedidaService.showMessage('Unidade de Medida criada com sucesso!');
    });
  ...
  updateGridData(): void {
    this.unidadeMedidaService.update(this.currentRecord).subscribe(() => {
      this.unidadeMedidaService.showMessage('Unidade de Medida atualizada com sucesso!');
      this.onEdit = false;
    });
  }

  atualizar(row: UnidadeMedida): void {
  ...
  deleteGridData(id: number): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.unidadeMedidaService.delete(id)
          .subscribe(() => {
            this.unidadeMedidaService.showMessage('Unidade de Medida apagada com sucesso!');
